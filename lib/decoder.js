...
function analyzeSlot(arr) {
    ...
    if (minimal.length == 0) {
        ...
        slot.guess[`uint${minimal.length * 8}`] = byteArrayToInt(minimal)
...
export default function analyzeStorage(state) {
    ...
    for (let slotBytes of chop(state.data, 32)) {
        let currSlot = `0x${(startSlot + i * 32).toString(16)}`;
